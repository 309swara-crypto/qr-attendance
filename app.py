...
import qrcode, io, base64, os, csv, math
...
students = []  # {name,email,roll,class}
attendance = [] # {roll,name,class,subject,time,status,distance}
qr_store = {} # qr_id -> {subject,lat,lon,expiry}
...
def teacher_login():
    ...
    return "❌ Wrong ID/Password - Use dbms/dbms123, stqa/stqa123, ethical/ethical123"

@app.route('/teacher')
def teacher_panel():
    if 'teacher' not in session and 'teacher_email' not in session:
        return redirect('/')
    return render_template('teacher_dashboard.html', teacher=session.get('teacher','Teacher'))
...
def google_login():
    try:
        ...
        for s in students:
            if s['email']==email:
                session['roll']=s['roll']; session['class']=s['class']
...
def register():
    ...
    session['roll']=roll; session['class']=class_name
    return redirect('/student')
...
def gen_qr():
    ...
    lat=float(request.args.get('lat',20.99)); lon=float(request.args.get('lon',75.56))
    qr_id=datetime.now().strftime("%Y%m%d%H%M%S")
    qr_store[qr_id]={"subject":subject,"lat":lat,"lon:":lon,"lat":lat,"lon":lon,"expiry":datetime.now()+timedelta(minutes=10)}
    # QR data = link to scan
    data=f"{request.host_url}scan/{qr_id}"
    img=qrcode.make(data)
    buf=io.BytesIO(); img.save(buf,'PNG'); buf.seek(0)
    b64=base64.b64encode(buf.getvalue()).decode()
    return render_template('generate_qr.html', qr_code=b64, subject=subject, lat=lat, lon=lon, qr_id=qr_id)

@app.route('/scan/<qr_id>')
def scan_page(qr_id):
    info=qr_store.get(qr_id)
    if not info or datetime.now()>info['expiry']:
        return "❌ QR Expired ⏱️ Ask Teacher for new QR"
...
def verify():
    ...
    attendance.append({
        "roll":session.get('roll'),"name":session.get('name'),"class":session.get('class'),
        "subject":info['subject'],"time":datetime.now().strftime("%d-%m-%Y %H:%M:%S"),
        "status":status,"distance":f"{int(dist)}m"
    })
    if dist<=50:
        return f"<h1>✅ PRESENT - {int(dist)}m 📍 Within 50m 🎉</h1><a href='/student'>Back</a>"
    else:
        return f"<h1>❌ ABSENT - Too Far {int(dist)}m ⚠️ Must be within 50m 📍</h1><a href='/student'>Back</a>"
...
def d_att():
    ...
    return send_file(buf, as_attachment=True, download_name=f"attendance_{datetime.now().date()}.xlsx")
