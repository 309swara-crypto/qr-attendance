...
def teacher_page():
    ...
    try:
        ...
        ac = cur.fetchone()[0]
        cur.execute("SELECT * FROM students ORDER BY roll LIMIT 20")
...
@app.route('/student')
def student_page():
    if 'email' not in session and 'roll' not in session:
        return redirect('/')
    try:
        conn=get_db(); cur=conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        cur.execute("SELECT * FROM attendance WHERE roll=%s ORDER BY id DESC", (session.get('roll'),))
        at=cur.fetchall(); cur.close(); conn.close()
    except:
        at=[]
    return render_template('student_dashboard.html', name=session.get('name','Student'), roll=session.get('roll'), class_name=session.get('class'), attendance=at)
...
def unified_login():
    ...
    if role=='Teacher':
        if TEACHERS.get(r)==p:
            session['teacher']=r
            return redirect('/teacher')
        return "Wrong Teacher Password <a href='/'>Back</a>"
    try:
        conn=get_db(); cur=conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        cur.execute("SELECT * FROM students WHERE roll=%s",(r,))
        ...
        print("LOGIN ERROR:", e)
    return f"Roll {r} not found. Ask teacher to add you manually. <a href='/'>Back</a>"
...
def scan(qid):
    i=qr_store.get(qid)
    if not i or datetime.now()>i['expiry']:
        return "QR Expired - Generate new one"
...
def verify():
    ...
    try:
        conn=get_db(); cur=conn.cursor()
        cur.execute("SELECT * FROM attendance WHERE roll=%s AND qr_id=%s",(session.get('roll'),qid))
        if cur.fetchone():
            cur.close(); conn.close()
            return "Already Marked <a href='/student'>Back</a>"
...
def reg():
    try:
        conn=get_db(); cur=conn.cursor()
        cur.execute("INSERT INTO students VALUES (%s,%s,%s,%s,%s)",(request.form.get('roll'),session['name'],session['email'],request.form.get('class_name'),request.form.get('roll')))
        conn.commit(); cur.close(); conn.close()
        session['roll']=request.form.get('roll'); session['class']=request.form.get('class_name')
        return redirect('/student')
    except Exception as e:
